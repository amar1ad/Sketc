                ...
                steps:
                  ...
                  - name: Detect Java Version
                    ...
                    run: |
                      ...
                      # Look for Java compatibility properties
                      if grep -rQI "VERSION_11" . 2>/dev/null; then
                        java_ver="11"
                      fi
                      if grep -rQI "VERSION_1_8" . 2>/dev/null; then
                        java_ver="8"
                      fi
                      if grep -rQI "VERSION_17" . 2>/dev/null; then
                        java_ver="17"
                      fi
                      if grep -rQI "VERSION_21" . 2>/dev/null; then
                        java_ver="21"
                      fi
                      
                      # Try Version catalogs first (libs.versions.toml) recursively to check AGP version and increase java requirement if needed
                      ...
                      echo "Detected JDK requirement: ${"$"}{java_ver}"
                  ...
                  - name: Build with Gradle
                    run: |
                      ...
                      # Remove local.properties to prevent local hardcoded SDK/NDK paths from breaking compilation
